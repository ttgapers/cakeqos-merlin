...
cake_download() {
	if [ "${1}" = "update" ]; then
		...
		# todo: compare version numbers (local vs repo) and update the script if different
		/usr/sbin/curl --retry 3 "https://raw.githubusercontent.com/ttgapers/cakeqos-merlin/develop/cake-qos.sh" -o "/jffs/scripts/${SCRIPT_NAME}" && chmod 0755 "/jffs/scripts/${SCRIPT_NAME}"
	elif [ "${1}" = "install" ]; then
	...
	if [ "${VERSIONS_ONLINE}" != "" ] && [ "${VERSION_LOCAL_CAKE}" != "" ] && [ "${VERSION_LOCAL_TC}" != "" ]; then
		...
		if [ "${VERSION_LOCAL_CAKE}" != "${VERSION_ONLINE_CAKE}" ] || [ "${VERSION_LOCAL_TC}" != "${VERSION_ONLINE_TC}" ]; then
			...
			if [ -f "/tmp/home/root/${FILE1_OUT}" ] && [ -f "/tmp/home/root/${FILE2_OUT}" ]; then
				...
				echo "There was an error downloading the binaries, please try again."
				return 1
			fi
		else
